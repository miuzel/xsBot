	...
	if ( msg.toLowerCase().startsWith('请你转播')) {
		...
		voiceChannel.join().then(async c => {
            try {
                ...
                stream = ytdl.downloadFromInfo(info, livequality.length ? { quality: livequality , highWaterMark: 1<<21, liveBuffer: 25000, begin: Date.now() } : {highWaterMark: 1<<21,  liveBuffer: 25000, quality: "lowestaudio", filter: 'audioonly' });
